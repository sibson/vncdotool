...
Credentials never reach disk, because the auth exchange never reaches disk.
The archive does not record the handshake that happened; it records a
``none``-auth one in its place -- the server's real greeting, a ``none``-only
security list, then the real session from ServerInit on. Whatever the server
demanded, and whatever you typed to satisfy it, is simply absent: not zeroed,
not shortened, gone.

That is also why the capture replays anywhere. Nobody needs your password to
look at your bug, so the archive describes a session that does not have one.

Stripping the exchange means knowing where it ends, which means being able to
follow it. vncdotool can follow ``none``, VNC authentication, and ARD / Apple
Screen Sharing, and cannot follow ``tight``, ``vencrypt``, ``rsa-aes`` or
UltraVNC's MS-Logon. A session negotiating one of those aborts the capture
rather than guess; the error says how to override it.

``meta.json`` still records which security types the server offered and which
one the session really negotiated. That is evidence about the server, and it
carries no secret.
...
was -- the same password, the same security type. It also replays whatever
credentials that exchange carried, which is why ``--server`` binds to
``127.0.0.1``. Pass ``--bind`` only for a capture you know carries nothing
sensitive.
