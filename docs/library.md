...
vncdotool is built with the Twisted framework, as such it best integrates with other Twisted Applications.
...
The first argument passed to the `api.connect` method is the VNC server to connect to, and it needs to be in the format `address[:display|::port]`.
For example:
...
`vncdotool.client.VNCDoToolClient` and they will block until completion.
For example:
...
In case of too many timeout errors, it is recommended to reset the client connection via the `disconnect` and `api.connect` methods.
