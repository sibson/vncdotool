...
class TestDesktopSize(unittest.TestCase):
    ...
    def test_desktop_size_resizes_screen_and_is_not_a_content_rect(self) -> None:
        ...
        self.client.dataReceived(framebuffer_update([raw_rect, resize_rect]))

        self.assertIsNotNone(self.client.screen)
        self.assertEqual(self.client.screen.size, (8, 6))
        self.assertEqual(self.client.rectanglePos, [(0, 0, 4, 4)])
...
class TestQemuExtendedKey(unittest.TestCase):
    ...
    def test_qemu_extended_key_is_negotiated_and_is_not_a_content_rect(self) -> None:
        handshake(self.client, 2, 2)
        self.assertNotIn(
            Encoding.PSEUDO_QEMU_EXTENDED_KEY_EVENT, self.client.negotiated_encodings
        )

        qemu_rect = rect(0, 0, 0, 0, Encoding.PSEUDO_QEMU_EXTENDED_KEY_EVENT)
        self.client.dataReceived(framebuffer_update([qemu_rect]))
