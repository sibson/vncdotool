...
class _BaseDes(object):
    ...
    def _pad_data(self, data, pad, padmode):
        ...
        if padmode == PAD_NORMAL:
            ...
            if not pad:
                raise ValueError("Data must be a multiple of {} bytes in length. "
                                 "Use padmode=PAD_PKCS5 or "
                                 "set the pad character.".format(self.block_size))
    ...
    def crypt(self, data, crypt_type):
        ...
        if len(data) % self.block_size != 0:
            ...
            if not self.get_padding():
                raise ValueError("Invalid data length, data must be a multiple of {} bytes\n."
...
class TripleDes(_BaseDes):
    ...
    def set_mode(self, mode):
        """Sets the type of crypting mode, pyDes.ECB or pyDes.CBC"""
        _BaseDes.set_mode(self, mode)
        for key in (self.__key1, self.__key2, self.__key3):
            key.set_mode(mode)

    def set_padding(self, pad):
        """set_padding() -> bytes of length 1. Padding character."""
        _BaseDes.set_padding(self, pad)
        for key in (self.__key1, self.__key2, self.__key3):
            key.set_padding(pad)

    def set_pad_mode(self, mode):
        """Sets the type of padding mode, pyDes.PAD_NORMAL or pyDes.PAD_PKCS5"""
        _BaseDes.set_pad_mode(self, mode)
        for key in (self.__key1, self.__key2, self.__key3):
