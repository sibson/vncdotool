...
----------------------
  ...
  - Add Fence extension support (``PSEUDO_FENCE``): the client offers the encoding and answers a server's synchronisation request. TigerVNC fences every client that offers it, from the first message of the session, so a client that offers Fence without answering it ends the session on an unknown message. ``vnclog``'s proxy understands the message too, so a proxied session keeps recording correctly (@sibson, based on @TeofilisMartisius's #323)
