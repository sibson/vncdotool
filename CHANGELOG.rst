...
----------------------
  ...
  - Add ``vncdo --expect-fuzz N`` and ``--expect-blur RADIUS``, tuning how near the screen has to be for ``expect`` to call it a match. A blur is what lets ``expect`` match a screen a lossy encoding has moved, and so sequence a ``--jpeg-quality`` capture at all (@sibson)
