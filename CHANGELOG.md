...
- [WARNING] Captures omit the mouse pointer by default. Add `vncdo --cursor {none,server,local}` and `api.connect(cursor=)`: `none` asks for the Cursor pseudo-encoding so servers that painted a pointer (x11vnc, libvncserver, and the Windows servers) stop, `server` asks for nothing and lets the server paint as before, `local` draws the shape the server sends -- pixel for pixel what `server` captures on x11vnc and libvncserver. `--localcursor` and `--nocursor` stay accepted as aliases for `local` and `none`. A library caller could not set this at all before, since `ThreadedVNCClientProxy` has no `__setattr__` (@sibson, #206). The PointerPos pseudo-encoding (-232) is offered and decoded alongside Cursor, which is what makes this work on UltraVNC: it revokes Cursor at the end of `SetEncodings` unless both were asked for, and went on painting the pointer without it. A server that moves the pointer itself now moves the `local` composite with it (@sibson)
