...
- Add VeNCrypt (security type 19), so a TLS-fronted server is no longer refused as an unknown security type (#310, #138). Every TLS-backed subtype is negotiated, over the new `vncdotool[tls]` extra; the bare `Plain` subtype, which puts the username and password on an unencrypted socket, is refused rather than offered a flag, and SASL and Ident are unimplemented. An X509 server certificate is verified by default, against the platform trust store and the address dialled; `vncdo --tls-ca-cert FILE` supplies a private CA instead, and `--tls-insecure-skip-verify` accepts a server whose identity cannot be established, which the anonymous `TLS*` subtypes always are. Driven live against TigerVNC and against wayvnc, the server #310 reports, which offers VeNCrypt alone and settles on `X509Plain`; Proxmox (#138) is still untested
