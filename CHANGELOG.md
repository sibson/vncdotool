...
- Fix `capture` writing a part-black screenshot against a server that answers a full-screen framebuffer update request with only part of the framebuffer, UltraVNC among them. A non-incremental refresh now waits until the rectangles it has been sent cover the screen, asks once more if they do not, and says how many pixels it is capturing as black rather than saving them silently (@sibson)
