...
- [WARNING] `vncdo` offers `tight,zrle,hextile,raw` instead of Raw alone, sending roughly a quarter of the bytes against a server that speaks Tight, at more CPU to decode. Every encoding on the list is lossless, so captures stay exact; `--encodings` still replaces the list, and `VNCDoToolClient.encoding` is gone (@sibson)
