...
- Fix `capture` ending in an `AssertionError` (exit 30) when a server answers the first framebuffer update request with pseudo-encoding rectangles alone, as TigerVNC on Windows does with a cursor shape; a refresh now waits for a rectangle that paints the framebuffer (@sibson)
