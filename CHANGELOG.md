...
- vncdo decodes framebuffer updates through a new pluggable per-encoding architecture (`vncdotool/decoders/`) instead of one monolithic protocol handler. CopyRect, RRE, CoRRE, Hextile and ZRLE go from compiled-but-unreachable to actually usable, and Tight is entirely new support (closes #264); server-to-client messages (`SetColourMapEntries`, `ServerCutText`, `Bell`, `ServerFence`) move to the same per-message-handler pattern (#474)
