...
- [WARNING] Captures omit the mouse pointer: `vncdo` always offers the Cursor pseudo-encoding, so servers that painted one (x11vnc, libvncserver) stop. `--localcursor` draws it back, `--nocursor` is accepted and now describes the default, and `api.connect()` callers get it without the flag they could never set (@sibson, #206). The PointerPos pseudo-encoding (-232) is offered and decoded alongside it, which is what makes this work on UltraVNC -- it revokes Cursor at the end of `SetEncodings` unless both were asked for, and went on painting the pointer without it. A server that moves the pointer itself now moves the `--localcursor` composite with it (@sibson)
