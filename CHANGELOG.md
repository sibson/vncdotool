...
- Fix `--cursor local` leaving a trail of stale pointers. The shape was pasted into the framebuffer itself, so an incremental `capture`, an `expect` or a `stable` could see one arrow per position the pointer had been, and every comparison ran against a framebuffer with client-drawn pixels in it. The framebuffer now holds the server's pixels alone and the pointer is composited onto a copy when a capture or a comparison is taken; `VNCDoToolClient.drawCursor()` is gone (@sibson, #522)
