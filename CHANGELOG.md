...
- Fix `--cursor local` leaving a trail of stale pointers. The shape was pasted into the framebuffer itself, so an incremental `capture`, an `expect` or a `stable` could see one arrow per position the pointer had been, and every comparison ran against a framebuffer with client-drawn pixels in it. The framebuffer now holds the server's pixels alone and the pointer is composited onto a copy when a capture or a comparison is taken. `VNCDoToolClient.renderScreen()` and `renderRegion(x, y, w, h)` hand a library caller that same image as a PIL `Image`, without the PNG round-trip `captureScreen` needs; `VNCDoToolClient.drawCursor()` is gone (@sibson, #522)
- A server reporting that the pointer moved -- a person at the physical keyboard, another client sharing the session, or the desktop warping it -- now moves where the next `click` goes, not just where `--cursor local` draws the arrow. `client.x`/`client.y` is the pointer's position and the only answer to where it is; a script that wants a click elsewhere moves there first (@sibson, #522)
- Fix `--cursor local` drawing the arrow -- and, since #522, the next `click` landing -- at the position the pointer held before the `move` that had just been issued. x11vnc polls the X pointer rather than reading back the event it acted on, so it can report a pre-move position as though something else had warped the pointer; a PointerPos arriving within a second of this client's own `move` is now discarded. See `specs/cursor.md` (@sibson, #527)
...
- Add `vncdo resize WIDTH HEIGHT`, asking the server to change its desktop size and exiting 30 if it refuses or never answers. `vncdo` now offers the `ExtendedDesktopSize` pseudo-encoding by default, so a server that speaks it sends the extended rectangle in place of the plain `DesktopSize` one; `--disable-desktop-resizing` turns off both. `libvncserver` and `x11vnc` advertise the encoding but refuse the resize itself (#301)
...
- Fix `capture` writing a part-black screenshot against a server that answers a full-screen framebuffer update request with only part of the framebuffer, UltraVNC among them. A non-incremental refresh now waits until the rectangles it has been sent cover the screen, re-asking for as long as each answer covers more of it than the last, and says how many pixels it is capturing as black rather than saving them silently. TightVNC's own capture can dribble content across three or more such answers under load; the earlier fix here only budgeted one extra request regardless of whether it made progress, which is the intermittent flat capture the Windows OS-server job kept seeing from TightVNC even after #528 (@sibson)
- Fix `capture` against a server that answers the first framebuffer update request with pseudo-encoding rectangles alone, as TigerVNC and TightVNC on Windows do by sending a cursor shape ahead of any screen content: it ended in an `AssertionError` (exit 30), or wrote an all-black screenshot and exited 0. A non-incremental refresh now waits for a rectangle that paints the framebuffer, and a Cursor rectangle -- a shape and a hotspot, not a screen region -- is no longer mistaken for one (@sibson)
