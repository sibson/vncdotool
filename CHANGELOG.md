...
- [WARNING] Captures omit the mouse pointer: `vncdo` always offers the Cursor pseudo-encoding, so servers that painted one (x11vnc, libvncserver) stop. `--localcursor` draws it back, `--nocursor` is accepted and now describes the default, and `api.connect()` callers get it without the flag they could never set (@sibson, #206)
