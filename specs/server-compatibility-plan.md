...
- ~~**WebSocket transport** (`ws://`/`wss://` server addresses) targeting
  ...
  autobahn behind the `websocket` extra, and exercised against three
  independent implementations of the handshake -- QEMU's built-in server,
  Selenoid's Go bridge and KasmVNC. Still open: `vnclog`
